...
1. `AgentRun` is a namespaced `control.benchhandoff.dev/v1alpha1` custom
   ...
   deadline.
2. The Go manager reconciles one deterministic Kubernetes Job at a time. It
   watches `AgentRun` and owned Job objects, and uses uncached API reads for
   Job and Pod identity decisions.
3. The Python `benchhandoff.controller_step` bridge performs exactly one
...
4. The PVC contains `suites/` and `runs/`. The run directory is keyed by the
...
This is tested restart/adoption behavior for one manager in one kind cluster.
It is not a proof of highly available leader election, multi-replica safety,
multi-cluster recovery, network-partition tolerance, or distributed fencing.
...
- PVC snapshots, storage fencing, remote leases, or protection from a hostile
  writer;
- multi-replica high availability, leader-election evidence, or multi-cluster
  coordination;
